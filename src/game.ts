...
export function createGame(player: Player, computer: Player): Game {
  addMsg(
    "Welcome to Battleship. Please initate attack on your opponent's waters.",
  );
  return {
    player,
    computer,
    ...
    compPlay() {
      setTimeout(() => {
        ...
        const sq = Array.from(pBoard.getElementsByClassName("bodySq")).filter(
          (el) => {
            return (
              Number(el.getAttribute("row")) === row &&
              Number(el.getAttribute("col")) === col
            );
          },
        )[0];
